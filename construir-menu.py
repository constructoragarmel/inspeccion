...
.clave-caja{flex:1;display:flex;align-items:stretch;border:1.5px solid var(--borde);border-radius:8px;background:#fff;overflow:hidden}
.clave-caja input{flex:1;min-width:0;min-height:44px;font-size:16px;padding:0 12px;border:none;background:transparent}
.clave-caja .ojo{width:44px;min-height:44px;border:none;border-left:1px solid var(--borde);background:#f4f6fa;font-size:18px;padding:0}
...
// Ver lo que se escribió: la clave lleva puntos y signos, y a ciegas se escribe mal.
function verClave(){
  const inp = document.getElementById('clave-menu');
  const oculta = inp.type === 'password';
  inp.type = oculta ? 'text' : 'password';
  document.getElementById('btn-ojo').textContent = oculta ? '🙈' : '👁';
  inp.focus();
}
...
function pintar(){
  ...
  document.getElementById('estado').innerHTML = hay
    ...
    : '⚠️ Este teléfono <b>todavía no está configurado</b>: se llenan informes igual, ' +
      ...
      '<button type="button" class="ojo" id="btn-ojo" onclick="verClave()" title="Ver la clave" aria-label="Ver la clave">👁</button></div>' +
