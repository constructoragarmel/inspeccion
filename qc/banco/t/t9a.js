...
await esperar(2400);
ok('A los 2 s se guardó solo', !sucio && listaGuardada().length === 1 && listaGuardada()[0].general[0].items[0].cant === '5');
...
ok('Al irse a otra app se guarda en el acto', !sucio && listaGuardada()[0].general[0].items[1].cant === '6');
...
ok('Informe con sección desconocida «urb_x_telecom»: se crea, con Fibra 300 m R y su nota', GENERAL.some(g => g.id === 'urb_x_telecom' && g.nombre === '9. TELECOM') && items('urb_x_telecom').some(it => it.querySelector('.nombre-libre').value === 'Fibra' && it.querySelector('.cant').value === '300' && it.querySelector('.ud-sel').value === 'm' && valorSN(it) === 'R') && $('#srv-urb_x_telecom .obs-srv').value === 'nota telecom');
