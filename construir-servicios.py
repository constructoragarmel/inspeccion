...
header .inicio{position:absolute;right:12px;top:9px;color:#fff;text-decoration:none;font-size:13px;font-weight:700;background:rgba(255,255,255,.16);border:1px solid rgba(255,255,255,.3);border-radius:8px;padding:0 10px;min-height:44px;display:flex;align-items:center;gap:4px}
...
function alElegirTorre(){
  ...
  if (filas.length === 1) {
    ...
    aviso.innerHTML = '<div class="aviso">⚠️ <b>' + escapar(t) + '</b> figura en el maestro con ' +
...
// Texto que va dentro de HTML: el nombre de una manzana lo escribe el inspector.
function escapar(v){ return String(v == null ? '' : v).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;'); }
...
function quitarApto(btn){
  ...
  // Las imágenes van en IndexedDB por posición (apto:0, apto:1…): al quitar una
  // fila las demás se corren y hay que reescribirlas, o la que sube se queda con
  // las fotos de la que se fue. Lo encontró el QC de incidencias del 21-sep.
  _fotosSucias = true;
  marcar();
}
...
function ofrecerHistorial(){
  ...
  caja.innerHTML = '<div class="historial">' +
    '<div class="t">📋 ' + escapar(t) + ' ya tiene un informe anterior</div>' +
